...
# 模型主要与数据库的表项有关，每一个类对应数据库里的一个表。
# 每个表备注了使用的基本信息，更多详见开发文档《智能家居云平台实现》。
# 注：每个模型重定义的__str__或__unicode__返回值不能以id结尾，会报错。
#     在设计文档中命名表的变量和实际开发的有区别，具体是前面加了一个“sh_”,保证变量唯一。
...
class tb_user(models.Model):
    ...
    sh_about = UEditorField('内容', height=300, width=1000,default=u'',
            blank=True, imagePath="uploads/images/",toolbars='besttome', 
            filePath='uploads/files/')                  # 用户信息的描述
    def __str__(self):
        return self.sh_username
# 模型必须返回一个__str__或__unicode__,__unicode__如下：
...
class tb_device(models.Model):
    ...
    sh_about = UEditorField('内容', height=300, width=1000,default=u'',
            blank=True,imagePath="uploads/images/",toolbars='besttome',
            filePath='uploads/files/')              # 设备表信息描述
    def __str__(self):
        return self.sh_name
...
class tb_sensor(models.Model):
    ...
    sh_about = UEditorField('内容', height=300, width=1000,default=u'',
            blank=True,imagePath="uploads/images/",toolbars='besttome',
            filePath='uploads/files/')              # 传感器信息的描述
    def __str__(self):
        return self.sh_name
...
class tb_datapoint_list(models.Model):
    ...
    sh_value = UEditorField('内容', height=300, width=1000,default=u'',
            blank=True,imagePath="uploads/images/",toolbars='besttome', 
            filePath='uploads/files/')              # 数据值
    def __str__(self):
